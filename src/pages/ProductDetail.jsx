...
import { ShieldCheck, Plus, Minus, ArrowRight, Award, Ruler, Percent } from 'lucide-react';
...
export default function ProductDetail() {
  ...
  // Derive sizes list (fallback if not structured, just display size)
          ...
          <div className="lg:col-span-5 bg-white p-8 sm:p-10 rounded-3xl border border-border/40 shadow-sm">
              ...
              <div className="flex gap-2.5">
                {sizes.map((sz) => (
                  <button
                    ...
                    className={`
                      w-11 h-11 text-xs font-bold rounded-xl border flex items-center justify-center transition-all
                      ${activeSize === sz 
            ...
            <div className="mb-8 p-5 bg-surface rounded-2xl border border-border/30">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-xs font-extrabold text-charcoal">{product.conditionScore || 90}/100 ({product.condition})</span>
              </div>
              {/* Progress bar */}
              <div className="w-full h-2 bg-border/40 rounded-full overflow-hidden">
                <div 
                  className="h-full bg-accent transition-all duration-500 rounded-full" 
                  style={{ width: `${product.conditionScore || 90}%` }}
                />
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3.5 mb-8">
              <button className="btn-primary w-full py-4 text-xs tracking-widest shadow-md">
